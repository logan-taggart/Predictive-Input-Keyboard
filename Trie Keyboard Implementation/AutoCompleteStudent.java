 ...
 * Builds GUI, Initializes Trie, Handles User Input, and Updates GUI Output <br> <br>
 * This app shows word predictions as you type. You can choose a prediction by
...
class AutoCompleteStudent extends JFrame implements KeyListener {
	...
	Trie2 trie; // This is the prefix trie data structure we will use to store the words
	...
	/**
	 * Sets up the GUI and loads the dictionary of WordItems into a Trie
	...
	public AutoCompleteStudent(WordItem[] worditemDictionary) {
		JFrame frame = new JFrame("Predictive Input Application (Trie)"); // Setup application GUI
		...
		trie = new Trie2(worditemDictionary); // Initialize new trie

		for (WordItem wordItem : worditemDictionary) { // Insert each word from the dictionary into the trie
			trie.insertString(wordItem.getWord());
		}
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (index < 0) {
			...
			if (Character.isAlphabetic(ch) && inWord) {
				...
				mostFrequentlyUsedWords = trie.insertString(partialWord); // Retrieve up to 9 of the most frequent words associated with the prefix we typed
		...
		} else if (index <= 9) { // Handles if the key pressed is either an enter, space, or numbers
	...
	/**
	 * Private function maps key events into key codes so they can be used for processing <br>
	...
	/**
	 * Private function to create a String of up to 9 of the most frequently used words for a given prefix <br>
	 ...
	 * @param fileName String representing the filepath to the dictionary that we are reading from
	 ...
	 * Initializes the prefix trie and launches the GUI <br>
	...
	public static void main(String[] args) throws IOException {
		System.out.println("Initializing with Trie...");
		...
		new AutoCompleteStudent(dict); // Initialize Trie and program with the extracted WordItems
